...
\begin{itemize}[leftmargin=1.4em, itemsep=0.2em]
  ...
  \item Report only verified results. Every number in the tables below was re-derived from the repository by re-running the JARS ODE on the final selected sets, and every cross-model intersection was recomputed from the saved CSVs. Earlier drafts of this report contained a handful of small but real errors that have been corrected here --- those corrections are listed explicitly in Appendix~A.
\end{itemize}

Throughout the report I use the following convention: greedy, post-greedy 1-swap, and stingy backward elimination are evaluated under the full JARS equilibrium objective $F(S)$, while all MIQP variants optimize a quadratic plumbing surrogate $\tilde{F}(S)$ using AMPL and Gurobi. Both views are needed because the surrogate is fast and provable but lossy, and the ODE is faithful but expensive.
...
\]
where $P_0(k)$ is the external larval supply at site $k$, $P_1$ is the larval transport probability matrix between sites, and the exponent $1.72$ (a published parameter for oysters) makes dense, retentive clusters disproportionately valuable. This nonlinearity is the structural reason greedy heuristics undershoot and clustered selection methods do well.

\subsection{The optimization objective}

Let $A_k^*(S)$ be the equilibrium adult biomass at site $k$ when the ODE is integrated to $t=1000$ with reefs restored at exactly the sites in $S$. The true objective is
\[
...
\]
with binary selection variables $x_i$ and the cardinality constraint $\sum x_i = 25$. There are four variants.
...
Adds a continuous reef-area variable $s_i \in [L_i, U_i]$ for each selected site ($5$--$50$ acres), with a global budget $\sum s_i \leq T = 1000$ acres and a normalization $\bar{S} = 20$. The objective scales each $Pe(i)$ and $W(i,j)$ entry by $s_i / \bar{S}$. This lets the model allocate big reefs to hubs and small reefs to marginal sites.

\subsubsection*{(d) MIQP + community + sizing --- \texttt{oyster\_comm\_size.mod}}
Combines (b) and (c). This is the model that most closely matches real planning constraints --- it must include some sites for regional equity, but it can size those sites small to limit the cost in surrogate larvae production.
...
\midrule
M1 Greedy & \texttt{10,\ 40,\ 41,\ 31,\ 32,\ 36,\ 37,\ 20,\ 47,\ 53,\ 15,\ 51,\ 52,\ 28,\ 49,\ 21,\ 27,\ 19,\ 30,\ 4,\ 26,\ 17,\ 16,\ 44,\ 54} & 1.846640 \\
...
\midrule
M2 Greedy & \texttt{10,\ 11,\ 12,\ 15,\ 16,\ 17,\ 19,\ 20,\ 26,\ 29,\ 31,\ 32,\ 33,\ 36,\ 37,\ 40,\ 41,\ 44,\ 47,\ 49,\ 51,\ 52,\ 53,\ 54,\ 59} & 1.692871 \\
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.2em]
  ...
  \item On Matrix~2, greedy already finds the same $25$ sites as the swap and stingy methods; the swap pass only improves the score by $\sim 2.4\%$ by reordering effects, not by changing the set.
\end{itemize}
...
All four AMPL/Gurobi models solved to proven optimality in well under a second on a standard laptop. Table~\ref{tab:miqp} summarizes the objective values and selected sets.

\begin{table}[H]
\centering
\caption{MIQP surrogate results under constant $P_0$.}
\label{tab:miqp}
\small
\begin{tabularx}{\linewidth}{@{}l X r@{}}
\toprule
\textbf{Model} & \textbf{Selected sites (sorted)} & \textbf{$\tilde{F}(S)$} \\
\midrule
M1 Base MIQP & \texttt{10,\ 11,\ 12,\ 15,\ 16,\ 17,\ 20,\ 26,\ 27,\ 28,\ 29,\ 31,\ 32,\ 33,\ 36,\ 37,\ 40,\ 41,\ 44,\ 49,\ 51,\ 52,\ 53,\ 54,\ 59} & 13692.07 \\
M1 MIQP + Comm & \texttt{10,\ 11,\ 12,\ 15,\ 16,\ 17,\ 21,\ 27,\ 28,\ 29,\ 31,\ 32,\ 33,\ 36,\ 37,\ 40,\ 41,\ 42,\ 47,\ 49,\ 51,\ 52,\ 53,\ 56,\ 59} & 12782.91 \\
M1 MIQP + Size & \texttt{10,\ 11,\ 12,\ 15,\ 16,\ 17,\ 20,\ 26,\ 27,\ 28,\ 29,\ 31,\ 32,\ 33,\ 36,\ 37,\ 40,\ 41,\ 44,\ 49,\ 51,\ 52,\ 53,\ 54,\ 59} & 58564.19 \\
M1 MIQP + Comm + Size & \texttt{10,\ 12,\ 15,\ 16,\ 17,\ 21,\ 26,\ 28,\ 29,\ 31,\ 32,\ 33,\ 37,\ 40,\ 41,\ 42,\ 44,\ 47,\ 48,\ 49,\ 51,\ 52,\ 53,\ 54,\ 59} & 56442.63 \\
\midrule
M2 Base MIQP & \texttt{10,\ 11,\ 12,\ 15,\ 16,\ 17,\ 20,\ 26,\ 27,\ 28,\ 29,\ 31,\ 32,\ 33,\ 36,\ 37,\ 40,\ 41,\ 44,\ 49,\ 51,\ 52,\ 53,\ 54,\ 59} & 15806.74 \\
M2 MIQP + Comm & \texttt{10,\ 12,\ 15,\ 16,\ 21,\ 26,\ 27,\ 28,\ 29,\ 31,\ 32,\ 33,\ 37,\ 40,\ 41,\ 42,\ 44,\ 47,\ 48,\ 49,\ 51,\ 52,\ 53,\ 54,\ 59} & 14587.09 \\
M2 MIQP + Size & \texttt{10,\ 11,\ 12,\ 15,\ 16,\ 17,\ 20,\ 26,\ 27,\ 28,\ 29,\ 31,\ 32,\ 33,\ 36,\ 37,\ 40,\ 41,\ 44,\ 49,\ 51,\ 52,\ 53,\ 54,\ 59} & 63607.36 \\
M2 MIQP + Comm + Size & \texttt{10,\ 11,\ 12,\ 15,\ 16,\ 21,\ 26,\ 27,\ 28,\ 29,\ 31,\ 32,\ 33,\ 36,\ 37,\ 40,\ 41,\ 42,\ 47,\ 48,\ 49,\ 51,\ 52,\ 53,\ 59} & 64447.23 \\
\bottomrule
\end{tabularx}
\end{table}

Two notes on the sizing models. First, in both matrices the optimal \emph{set} under sizing is identical to the base MIQP set --- the optimizer doesn't change who is selected, it just allocates large area ($40$--$50$ acres) to high-connectivity hubs and small area ($5$--$15$ acres) to weaker neighbors. Second, the total reef area is exactly $1000$ acres in every sizing solution; the budget constraint is tight in all cases.

Representative size profile (M1 MIQP + Size): $50$ acres at sites $10, 11, 12, 15, 16, 17, 26$--$33$; $40$ acres at sites $40, 41, 44, 51$--$54, 59$; and $5$--$15$ acres at sites $20, 36, 37, 49$. See \texttt{runs/miqp\_size\_sites\_matrix\{1,2\}.csv} for the full per-site allocations.

\subsection{Backbone sites: intersections across models and matrices}
\label{sec:backbone}

The point of running so many models is to see which sites are robust to the choice of method, matrix, and forcing regime. The intersection structure is the cleanest summary.

\subsubsection*{Within-matrix constant-$P_0$ intersection (all $7$ constant models)}
...
\midrule
Matrix 1 & \texttt{10, 15, 16, 17, 28, 31, 32, 37, 40, 41, 49, 51, 52, 53} & 14 \\
Matrix 2 & \texttt{10, 12, 15, 26, 29, 31, 32, 37, 40, 41, 49, 51, 52, 53} & 14 \\
\bottomrule
\end{tabular}
\end{table}

Each matrix has a $14$-site backbone across its seven constant-forcing optimization runs.
...
\midrule
Intersection of M1 (14) and M2 (14) backbones & \texttt{10, 15, 31, 32, 37, 40, 41, 49, 51, 52, 53} & 11 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection*{Global backbone (all $18$ models, both matrices, both $P_0$ regimes)}
...
\textbf{Selection} & \textbf{Sites} & \textbf{Size} \\
\midrule
Sites selected by every one of the $18$ model runs & \texttt{10, 31, 37, 40, 41, 49, 53} & 7 \\
\bottomrule
\end{tabular}
\end{table}

These seven sites --- \textbf{10, 31, 37, 40, 41, 49, 53} --- appear in every greedy, every greedy+swap, every stingy, and every MIQP variant (base, +Comm, +Size, +Comm+Size) on both connectivity matrices and under both constant and realistic external forcing. They are the structural arteries of the bay's larval network: any restoration design that excludes them is structurally weaker.

\subsection{Network centrality on the constant-\texorpdfstring{$P_0$}{P0} backbone}

To explain why the same sites keep being selected, we computed five weighted centrality metrics on the directed connectivity graph defined by $P_1$ for each matrix: in-strength, out-strength, betweenness (with edge length $1 / \text{weight}$), eigenvector centrality, and PageRank. Sites were ranked separately by each metric, and the average rank gives an overall ``network importance'' ordering. Tables~\ref{tab:netM1} and \ref{tab:netM2} show the top $10$ sites of the $11$-site cross-matrix backbone for each matrix.

\begin{table}[H]
\centering
\caption{Matrix 1 --- top $10$ of the backbone by average rank.}
\label{tab:netM1}
\small
\begin{tabular}{@{}rrrrrrr@{}}
\toprule
\textbf{Site} & \textbf{In-str.} & \textbf{Out-str.} & \textbf{Betw.} & \textbf{Eig.} & \textbf{PR} & \textbf{avg rank} \\
\midrule
10 & 359,474 & 214,957 & 0.0266 & 0.5974 & 0.0999 & 7.0 \\
40 & 292,701 & 218,022 & 0.0541 & 0.6304 & 0.0936 & 4.8 \\
32 & 198,442 & 226,987 & 0.0966 & 0.1119 & 0.0273 & 6.0 \\
31 & 173,926 & 144,740 & 0.0545 & 0.2975 & 0.0584 & 7.8 \\
41 & 166,988 & 193,584 & 0.0306 & 0.2877 & 0.0456 & 10.0 \\
53 & 93,500  & 123,635 & 0.0842 & 0.0466 & 0.0270 & 11.2 \\
52 & 93,545  & 153,534 & 0.0563 & 0.0078 & 0.0138 & 15.8 \\
15 & 194,082 & 179,547 & 0.0098 & 0.0206 & 0.0175 & 16.4 \\
49 & 79,623  & 95,582  & 0.0381 & 0.0174 & 0.0237 & 16.6 \\
37 & 120,416 & 20,909  & 0.0168 & 0.1453 & 0.0481 & 17.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Matrix 2 --- top $10$ of the backbone by average rank.}
\label{tab:netM2}
\small
\begin{tabular}{@{}rrrrrrr@{}}
\toprule
\textbf{Site} & \textbf{In-str.} & \textbf{Out-str.} & \textbf{Betw.} & \textbf{Eig.} & \textbf{PR} & \textbf{avg rank} \\
\midrule
31 & 195,232 & 179,626 & 0.0408 & 0.3092 & 0.0750 & 8.8 \\
40 & 423,838 & 115,054 & 0.0363 & 0.6913 & 0.1574 & 9.2 \\
10 & 541,264 & 90,109  & 0.0355 & 0.5889 & 0.1311 & 10.2 \\
49 & 130,852 & 155,422 & 0.0918 & 0.0178 & 0.0200 & 10.4 \\
36 & 119,622 & 163,586 & 0.0395 & 0.1133 & 0.0346 & 12.2 \\
41 & 161,622 & 234,275 & 0.0013 & 0.2140 & 0.0477 & 12.8 \\
37 & 120,277 & 37,213  & 0.0758 & 0.1031 & 0.0323 & 13.0 \\
53 & 87,729  & 196,023 & 0.0472 & 0.0183 & 0.0144 & 13.2 \\
32 & 224,566 & 162,872 & 0.0186 & 0.0290 & 0.0144 & 15.0 \\
15 & 159,499 & 156,282 & 0.0337 & 0.0094 & 0.0115 & 17.2 \\
\bottomrule
\end{tabular}
\end{table}

The pattern is consistent across both matrices: sites $10, 31, 32, 40, 41$ are simultaneously high on eigenvector centrality and PageRank, meaning they are embedded in dense subnetworks of other important nodes. Sites $36, 49, 53$ score relatively high on betweenness, indicating they act as bridges between otherwise weakly-connected regions. This is the network-theoretic explanation for why optimization keeps selecting them: the JARS dynamics reward retentive loops and bridges, both of which these sites provide.
...
\begin{lstlisting}
# 1. Generate AMPL data from the Excel matrix
...
Run times on a recent laptop: each MIQP variant solves in well under a second; each greedy or stingy run takes $\sim 4$--$5$ minutes (because every step calls the ODE); a full greedy + local swap run takes $\sim 15$--$30$ minutes depending on how many improving passes are needed.
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.2em]
  \item \texttt{scripts/network\_core\_analysis.py} hard-codes a \texttt{CORE\_23} list at the top. After this re-analysis, the correct unconstrained-models constant-$P_0$ backbone across both matrices is $22$ sites, not $23$ --- site $47$ is selected by the heuristics but not by the base MIQP or MIQP+Size on either matrix. A short comment to that effect would prevent the next person from re-deriving it.
...
Across two independent connectivity matrices, four MIQP variants, three ODE-driven heuristics, and two external forcing regimes --- $18$ distinct optimization runs in total --- the same seven reefs are selected by every single one: $10, 31, 37, 40, 41, 49, 53$. The eleven-site cross-matrix constant-forcing backbone $\{10, 15, 31, 32, 37, 40, 41, 49, 51, 52, 53\}$ is the next natural layer out, and the per-matrix $14$-site backbones extend that further.

That this much agreement emerges from objectives that are conceptually quite different --- the full nonlinear JARS equilibrium on one hand, a quadratic plumbing surrogate on the other --- is not a tautology. It tells us that the surrogate captures the dominant structural drivers of the metapopulation dynamics. From a planning perspective, the practical translation is: a small number of reefs do most of the connectivity work, and any restoration design that omits the seven-site backbone is, with high confidence, structurally worse than one that includes it.
...
\midrule
\endhead
M1 Greedy (constant) & \texttt{10, 40, 41, 31, 32, 36, 37, 20, 47, 53, 15, 51, 52, 28, 49, 21, 27, 19, 30, 4, 26, 17, 16, 44, 54} \\
...
M1 MIQP + Comm & \texttt{10, 11, 12, 15, 16, 17, 21, 27, 28, 29, 31, 32, 33, 36, 37, 40, 41, 42, 47, 49, 51, 52, 53, 56, 59} \\
M1 MIQP + Size & \texttt{10, 11, 12, 15, 16, 17, 20, 26, 27, 28, 29, 31, 32, 33, 36, 37, 40, 41, 44, 49, 51, 52, 53, 54, 59} \\
M1 MIQP + Comm + Size & \texttt{10, 12, 15, 16, 17, 21, 26, 28, 29, 31, 32, 33, 37, 40, 41, 42, 44, 47, 48, 49, 51, 52, 53, 54, 59} \\
...
M2 Greedy (constant) & \texttt{10, 11, 12, 15, 16, 17, 19, 20, 26, 29, 31, 32, 33, 36, 37, 40, 41, 44, 47, 49, 51, 52, 53, 54, 59} \\
...
M2 MIQP + Comm & \texttt{10, 12, 15, 16, 21, 26, 27, 28, 29, 31, 32, 33, 37, 40, 41, 42, 44, 47, 48, 49, 51, 52, 53, 54, 59} \\
M2 MIQP + Size & \texttt{10, 11, 12, 15, 16, 17, 20, 26, 27, 28, 29, 31, 32, 33, 36, 37, 40, 41, 44, 49, 51, 52, 53, 54, 59} \\
M2 MIQP + Comm + Size & \texttt{10, 11, 12, 15, 16, 21, 26, 27, 28, 29, 31, 32, 33, 36, 37, 40, 41, 42, 47, 48, 49, 51, 52, 53, 59} \\
